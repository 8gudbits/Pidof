...
void findPID(const std::string &filename, const std::string &title = "")
{
    ...
    {
        // Command for searching by window title for v1.1
    ...
    for (const auto &line : lines)
    {
        ...
        {
            // Print the table rows with PID first, Memory Usage second, and Image Name last
            std::cout << std::left << std::setw(10) << trimQuotes(columns[1])  // PID
                      << std::setw(15) << trimQuotes(columns[4])  // Memory Usage
                      << std::setw(20) << trimQuotes(columns[0])  // Image Name
                      << std::endl;
        }
    }
}
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 2) {
        std::cerr << "\npidof 1.2 (x64) : (c) 8gudbits - All rights reserved.\n"
    ...
    std::cout << "\n"
              ...
              << std::setw(20) << "Image Name" << std::endl;
